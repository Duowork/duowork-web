...
## Best Practices
1. Use signals for global state management
2. Use preact/compat for React compatibility
3. Use preact/hooks for hooks
4. Use preact/signals for signals
5. React compat for ecosystem — Alias react to preact/compat; use React component libraries without modification
6. 3kB advantage — Preact shines in performance-critical contexts: embedded widgets, mobile web, slow connections
7. Same API as React — useState, useEffect, useRef, useMemo all work identically; easy migration
8. No synthetic events — Preact uses native DOM events; slightly different from React's event system
9. Prerender for SSG — Use preact-render-to-string for server rendering; or use Fresh (Deno) for full SSR
10. HTM for no-build — Use htm tagged template literals instead of JSX; works without a build step