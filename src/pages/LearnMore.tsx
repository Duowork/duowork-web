...
export default function LearnMore() {
  const services = [
    {
      ...
      description:
        "Before building anything, we map your workflows, identify where your team is losing time, and deliver a ranked action plan — quick wins first.",
      result: [
    ...
    },
    {
      ...
      description:
        "We design and build web and mobile app products with intelligence built baked-in –– not bolted on. Product that learn, adapt, and get faster over time",
      result: [
  ...
  return (
    <section className="h-screen bg-no-repeat bg-top bg-cover relative bg-[url('/team.webp')] bg-black/80 bg-blend-multiply">
      <TopNav />

      <div className="px-6 py-16 md:px-12 lg:px-20 max-w-[1400px] mx-auto z-20 w-full text-black mt-12 mb-12 rounded-3xl shadow-2xl font-sans relative">
        <div className="flex flex-col gap-4 mb-16 max-w-5xl">
          <small className="text-duo-green-600 font-bold tracking-widest uppercase">
            What we do
          </small>

          <h1 className="text-4xl md:text-5xl lg:text-7xl font-semibold tracking-tight text-gray-900 leading-tight">
            We help organisations operate at AI speed
          </h1>

          <p className="text-lg md:text-xl lg:text-2xl text-gray-800 leading-relaxed max-w-6xl font-body">
            Not just websites. Systems, products, and workflows that compound over
            time.
          </p>
        </div>
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-16">
          {services.map((item, index) => (
            <div key={index} className="flex flex-col gap-4">
              <h3 className="text-2xl md:text-[2rem] font-medium tracking-tight text-gray-900 leading-tight">
                {item.title}
              </h3>

              <p className="text-gray-800 text-base md:text-lg leading-relaxed max-w-[500px] font-body">
                {item.description}
              </p>

              <ul className="flex flex-col gap-2 mt-2">
                {item.result.map((result, idx) => (
                  <li key={idx} className="text-gray-600 text-sm md:text-base flex items-start gap-2 font-body">
        ...
        <div className="mt-16 flex">
          <Button value="Contact Us" btnClass="cta-btn" isLink linkTo="/contact" />
        </div>
      </div>
    </section>
  )
